...
//        }else{
//            log.info("все плохо");
//            log.info(String.valueOf(userInfo.getClass()));
//            log.info((String) userInfo);
//        }
        UserInfo user = (UserInfo) session.getAttribute("userInfo");
//        log.info("411");
        Long userId = user.getId();
        log.info("42");// Получаем ID пользователя
        UserInfo currentUser = userRepository.findById(userId).orElseThrow(() -> new RuntimeException("User not found"));
...
//    public Map<String, Double> getAverageFuelConsumptionByMonthForUser(Long userId) {
//
...
//
//
//        Map<String, Double> averageConsumptionByMonth = new HashMap<>();
//
//
//        for (Car car : cars) {
//
//            List<Refueling> refuelings = refuelingRepository.findByCarId(car.getId());
//
//
//            Map<String, List<Refueling>> refuelingsByMonth = refuelings.stream()
//                    .collect(Collectors.groupingBy(
//                            refueling -> refueling.getRefuelDate().getMonth() + " " + refueling.getRefuelDate().getYear()
//                    ));
//
//
//            for (Map.Entry<String, List<Refueling>> entry : refuelingsByMonth.entrySet()) {
//                List<Refueling> refuelingsInMonth = entry.getValue();
//                double totalFuel = 0.0;
//                for (Refueling refueling : refuelingsInMonth) {
//                    totalFuel += refueling.getFuelQuantity();
//                }
//                double averageFuel = totalFuel / refuelingsInMonth.size();
//                averageConsumptionByMonth.put(entry.getKey(),
//                        averageConsumptionByMonth.getOrDefault(entry.getKey(), 0.0) + averageFuel);
//            }
//        }
//
//        for (Map.Entry<String, Double> entry : averageConsumptionByMonth.entrySet()) {
//            averageConsumptionByMonth.put(entry.getKey(), entry.getValue() / cars.size());
//        }
//
//        return averageConsumptionByMonth;
//    }
    ...
    public Map<YearMonth, Double> getFuelEfficiencyByMonthForUser(Long userId, LocalDate from, LocalDate to) {
        ...
        // Карта: месяц → общее количество литров
        Map<YearMonth, Double> totalFuelPerMonth = new HashMap<>();
        // Карта: месяц → общий пробег
        ...
        // Рассчёт расхода: (топливо / пробег) * 100
        ...
        for (YearMonth ym : sortedMonths) {
            ...
            efficiencyPerMonth.put(ym, Math.round(efficiency * 100.0) / 100.0); // округление до 2 знаков
        }
    ...
    public Map<YearMonth, Double> getFuelCostEfficiencyByMonthForUser(Long userId, LocalDate from, LocalDate to) {
        ...
        // Месяц → общие затраты на топливо (в рублях)
        Map<YearMonth, Double> totalCostPerMonth = new HashMap<>();
        // Месяц → суммарный пробег (в километрах)
        Map<YearMonth, Double> totalDistancePerMonth = new HashMap<>();
        ...
        // Вычисляем (рубли / км) * 100 = руб/100 км
        Map<YearMonth, Double> efficiencyPerMonth = new LinkedHashMap<>();
        ...
        for (YearMonth ym : sortedMonths) {
            ...
            efficiencyPerMonth.put(ym, Math.round(efficiency * 100.0) / 100.0); // округляем до 2 знаков
        }
